        ...
        rollupOptions: {
          output: {
            manualChunks: (id: string) => {
              ...
              // Qwik核心分割
              if (id.includes('@builder.io/qwik')) return 'qwik-core';

              // 大型组件分割
              if (id.includes('/calendar/')) return 'components-calendar';
              if (id.includes('/dashboard/')) return 'components-dashboard';
              if (id.includes('/notifications/')) return 'components-notifications';
              if (id.includes('/layout/')) return 'components-layout';
            }
          }
        },
        // 增加chunk大小警告限制到800KB（减少警告）
        chunkSizeWarningLimit: 800,
        // 启用压缩 - 使用esbuild（Vite默认，无需额外依赖）
        minify: 'esbuild'
      }
    };
  }
